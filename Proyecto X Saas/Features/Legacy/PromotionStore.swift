...
enum LegacyFoodCategory: String, CaseIterable {
    case pizza = "Pizza"
    case sushi = "Sushi"
...
// MARK: - LegacyPromotion Model
struct LegacyPromotion: Identifiable, Hashable {
    let id = UUID()
    let title: String
    let discount: String
    let description: String
    let storeName: String
    let category: LegacyFoodCategory
...
// MARK: - LegacyStore Model
struct LegacyStore: Identifiable, Hashable {
    let id = UUID()
    let name: String
    let description: String
    let category: LegacyFoodCategory
    ...
    let priceRange: LegacyPromoPriceRange
    let features: [String]
    let imageName: String
...
enum LegacyPromoPriceRange: String, CaseIterable {
    case budget = "$"
    case moderate = "$$"
    case expensive = "$$$"
    case luxury = "$$$$"
...
// MARK: - Data LegacyStore
class LegacyPromotionStoreData: ObservableObject {
    @Published var promotions: [LegacyPromotion] = []
    @Published var stores: [LegacyStore] = []
    ...
    func filteredPromotions(by category: LegacyFoodCategory) -> [LegacyPromotion] {
        if category == .all {
            return promotions
        }
    ...
    func filteredStores(by category: LegacyFoodCategory) -> [LegacyStore] {
        if category == .all {
            return stores
        }
