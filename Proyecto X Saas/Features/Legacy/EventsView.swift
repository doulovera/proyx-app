...
struct LegacyEventsView: View {
    @StateObject private var eventStore = EventStore()
    @State private var selectedFilter: EventFilter = .today
    @State private var selectedEvent: LegacyEvent?
    
    var filteredEvents: [LegacyEvent] {
    ...
    var body: some View {
        NavigationView {
            ...
            .sheet(item: $selectedEvent) { event in
                LegacyEventDetailView(event: event)
            }
        }
    }
    ...
    private var recommendedEvents: [LegacyEvent] {
        return eventStore.events.filter { $0.isSponsored }.prefix(4).map { $0 }
    }
}

struct LegacyEventDetailView: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: AppTheme.Spacing.md) {
                ...
                Text("Lugar: \(event.location)")
                    .bodyText()
                Text("Fecha: \(event.dateText) • \(event.time)")
                    .captionText()
                    .foregroundColor(AppTheme.Colors.secondaryText)
                Text("Precio: \(event.price == 0 ? "Gratis" : "€\(Int(event.price))")")
                    .bodyText()
                Text("Tickets disponibles: \(event.availableTickets)")
                    .bodyText()
                if !event.includes.isEmpty {
                    Text("Incluye:")
                        .bodyText()
