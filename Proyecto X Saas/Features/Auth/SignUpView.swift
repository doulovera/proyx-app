...
struct SignUpView: View {
    ...
    var body: some View {
        ...
        .onChange(of: [fullName, email, phoneNumber, dni, password, confirmPassword, address]) { _ in
            validateForm()
        }
        .onChange(of: acceptTerms) { _ in
            validateForm()
        }
    }
        ...
        VStack(spacing: AppTheme.Spacing.lg) {
            ...
            if let errorMessage, !errorMessage.isEmpty {
                Text(errorMessage)
                    ...
                    .cornerRadius(AppTheme.CornerRadius.small)
            } else if showingErrors && !isFormValid {
    ...
    private func performSignUp() {
        ...
        let lastName = parts.count > 1 ? String(parts[1]) : "User"
        ...
        Task {
            do {
                let response = try await dependencies.authService.register(request: request)
